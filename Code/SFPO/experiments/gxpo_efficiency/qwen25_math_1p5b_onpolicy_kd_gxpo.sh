#!/usr/bin/env bash
#
# qwen25_math_1p5b_onpolicy_kd_gxpo.sh
#
# Per-step ON-POLICY KD (Option B: separate teacher Ray group) + GXPO.
# Student: Qwen2.5-1.5B-Instruct.  Teacher: frozen Qwen2.5-Math-7B
...
# (rollout.n=1) -> teacher scores those exact sequences -> GXPO update on
# [B, R, K=32] -> next batch. Two 2 GPU-locked engines never coexist:
...
# subset of common.sh's environment preamble and the gxpo method flags it
# needs, then adds the +actor_rollout_ref.actor.use_kd=True... KD flags.
#
# Usage:
#   bash qwen25_math_1p5b_onpolicy_kd_gxpo.sh            # launch
#   bash qwen25_math_1p5b_onpolicy_kd_gxpo.sh --dry-run  # preflight only
set -euo pipefail
...
if [[ -z "${VIRTUAL_ENV:-}" ]]; then
  for _candidate_venv in       "$GXPO_PROJECT_ROOT/.venv"       "/office/dev_workspace/swapnil/gradient-extrapolation-based-policy-optimization-gxpo-speed-audit/.venv"       "$REPO_ROOT/.venv"; do
    if [[ -x "$_candidate_venv/bin/python" ]]; then
      export VIRTUAL_ENV="$_candidate_venv"
      export PATH="$_candidate_venv/bin:$PATH"
      break
    fi
  done
  unset _candidate_venv
fi
PYTHON_BIN="${PYTHON_BIN:-${VIRTUAL_ENV:-}/bin/python}"
# Ensure venv tools such as ninja are available even when VIRTUAL_ENV was
# provided by the caller rather than selected by the block above.
...
# OOM fix: 17GB was lost to allocator fragmentation on the failing run
# (single 18GB block failed against 17.6GB free). expandable_segments is NOT
# usable here -- vLLM's CuMemAllocator hard-crashes on it at engine init --
# so cap split sizes instead: blocks >=128MB stay intact for big allocations.
export PYTORCH_CUDA_ALLOC_CONF="${PYTORCH_CUDA_ALLOC_CONF:-max_split_size_mb:128}"
# Level 1 keeps student weights resident and discards only KV cache, so the
# wake path skips the full weight re-sync every step.
...
# ---------------------------------------------------------- run identity ----
MODEL_ALIAS="qwen25-1p5b"
MODEL_ID="${MODEL_QWEN25_MATH_1P5B:-Qwen/Qwen2.5-1.5B-Instruct}"
KD_SOURCE="${KD_SOURCE:-onpolicy}"
...
if [[ "$KD_SOURCE" == "offpolicy_oss" ]]; then
  ...
  RUN_NAME="${RUN_NAME:-${MODEL_ALIAS}_onpolicy_kd_gxpo_k3_a0.1_b256_topk16}"
fi
...
if [[ "$KD_SOURCE" == "offpolicy_oss" ]]; then
  TRAIN_FILES="${TRAIN_FILES:-['$DATA_ROOT/oss_low_kd/train_teacher_topk16.parquet']}"
  OFFPOLICY_FLAGS=(+data.off_policy_kd=True +actor_rollout_ref.rollout.off_policy=True +actor_rollout_ref.actor.kd_teacher_cache=True)
  KD_REVERSE_KL="False"  # off-policy fixed data is forward KL's home turf
  # Idle engine (never generates) but must still initialize its cache
  # structure for 32k context: 0.05 fails init, 0.15 works.
  [[ -n "${VLLM_GPU_MEMORY_UTILIZATION:-}" ]] || export VLLM_GPU_MEMORY_UTILIZATION=0.15
else
  TRAIN_FILES="${TRAIN_FILES:-['$DATA_ROOT/dapo_math/train.parquet','$DATA_ROOT/lighteval-math/train.parquet']}"
  OFFPOLICY_FLAGS=()
fi
# Traditional periodic-eval / checkpoint strategy (mirrors common.sh and the
# non-GXPO KD launcher): greedy-decode validation every TRAINER_TEST_FREQ
# steps on the standard 6-benchmark val set via the same vLLM rollout engine
# (it sleeps again once generation finishes and training resumes), the
# best-scoring checkpoint is saved to a single "best_checkpoint" dir that is
# overwritten whenever a new best is found, and the most recent
# "global_step_N" resumable checkpoint is saved every SAVE_FREQ steps with
# only the latest one kept (keep_last_ckpts=1 below), replacing the previous
...
VAL_FILES="${VAL_FILES:-['$MATH500','$AIME24','$AIME25','$AMC23','$MINERVA','$OLYMPIAD']}"
MAX_RESPONSE_LENGTH="${MAX_RESPONSE_LENGTH:-3072}"    # 1024 prompt + 3072 response (also caps greedy eval generations)
...
# --------------------------------------------------------------- GXPO -------
K="${K:-3}"
REPOSITION_ALPHA="${REPOSITION_ALPHA:-0.1}"
GXPO_TAU="${GXPO_TAU:-3.0}"
GXPO_ZSCORE_W="${GXPO_ZSCORE_W:-30}"
# The requested run keeps the extrapolated GXPO path active from step 1
# through the final step.  Force mode bypasses warmup and entropy shutoff.
GXPO_FORCE_ALL_STEPS="${GXPO_FORCE_ALL_STEPS:-True}"
...
KD_TOPK="${KD_TOPK:-16}"
# On-policy KD must use reverse (mode-seeking) KL: forward KL is
# mass-covering and pumps entropy once student prefixes push the teacher
# into flat, low-top-K-mass regimes. Default False preserves legacy runs.
...
if [[ "$DRY_RUN" -eq 1 ]]; then
  cat <<EOT
[dry-run] resolved on-policy KD+GXPO launch configuration
  repo_root          : $REPO_ROOT
  student            : $MODEL_ID
  ...
  optim              : lr=$LR
  gxpo               : K=$K alpha=$REPOSITION_ALPHA tau=$GXPO_TAU shutoff=$GXPO_SHUTOFF_MODE txn=$GXPO_OPTIMIZER_STATE_MODE
  kd                 : topk=$KD_TOPK use_pg=False reverse_kl=$KD_REVERSE_KL source=$KD_SOURCE
...
"$PYTHON_BIN" -u -m verl.trainer.main_ppo \
  ...
  +actor_rollout_ref.actor.gxpo_force_all_steps="$GXPO_FORCE_ALL_STEPS" \
  ...
  +actor_rollout_ref.actor.gxpo_binary_reward=True \
  +actor_rollout_ref.actor.gxpo_format_error_skip_enabled=False \
  +actor_rollout_ref.actor.use_kd=True \
  +actor_rollout_ref.actor.kd_use_pg=False \
  +actor_rollout_ref.actor.kd_topk="$KD_TOPK" \
  +actor_rollout_ref.actor.kd_reverse_kl="$KD_REVERSE_KL" \
  ...
  ${OFFPOLICY_FLAGS:+${OFFPOLICY_FLAGS[@]}} \
  2>&1 | tee "$RUN_DIR/train.log"

# Run one greedy six-benchmark evaluation only after global_step_200 is complete.
if [[ "${SKIP_EVAL:-0}" != 1 ]]; then
  "$PYTHON_BIN" -u scripts/eval_checkpoints.py \
    --run_dir "$RUN_DIR" \
    --base_model "$MODEL_ID" \
    --data \
      "$DATA_ROOT/math500/test.parquet" \
      "$DATA_ROOT/aime2024/test.parquet" \
      "$DATA_ROOT/aime2025/test.parquet" \
      "$DATA_ROOT/amc/test.parquet" \
      "$DATA_ROOT/minervamath/test.parquet" \
      "$DATA_ROOT/olympiadbench/test.parquet" \
    --steps 200 --n 1 --temperature 0 --max_tokens 3072 --tp 1 \
    --gpu_memory_utilization 0.85 \
    --out "$RUN_DIR/final_greedy_eval.csv" 2>&1 | tee "$RUN_DIR/final_greedy_eval.log"
fi

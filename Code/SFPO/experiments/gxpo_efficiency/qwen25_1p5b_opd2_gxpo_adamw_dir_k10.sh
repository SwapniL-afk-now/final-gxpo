...
#
# Qwen2.5-3B-Instruct (student) + Qwen2.5-Math-7B-Instruct (teacher) | GXPO + AdamW |
...
# Keep host RAM bounded: no dataloader subprocess copies or Ray default 52 GiB object store.
...
# mini=256 (full-batch) + seq-mean-token-mean: the paper-faithful settings from
# the most recent stable OPD^2 relaunch (fixes the length-weighted token-mean
# bias that drove the runaway on this trio).
...
# Offload AdamW optimizer state to host RAM.

...
RUN_NAME="${RUN_NAME:-${MODEL_ALIAS}_onpolicy_kd_nogxpo_control_b256_topk${KD_TOPK:-32}}"
...
MAX_RESPONSE_LENGTH="${MAX_RESPONSE_LENGTH:-2048}"    # 1024 prompt + 2048 response -> 3072 teacher max_model_len; matches reference kd150g8k
...
[dry-run] resolved on-policy KD+GXPO launch configuration
  ...
  optim              : lr=$LR
  gxpo               : K=$K alpha=$REPOSITION_ALPHA tau=$GXPO_TAU shutoff=$GXPO_SHUTOFF_MODE
...
mkdir -p "$RUN_DIR"
export WANDB_DIR="$RUN_DIR" WANDB_PROJECT="$PROJECT" WANDB_MODE="${WANDB_MODE:-offline}"
...
python -u -m verl.trainer.main_ppo \
  ...
  trainer.total_training_steps="${MAX_STEPS:-10}" \

...
#
# Qwen2.5-Math-1.5B-Instruct | GXPO + AdamW | TRANSACTIONAL optimizer state |
# OPTIMIZER-DIRECTION retention | K=10 | alpha=0.3 | batch 64 | minibatch 16.
#
# What makes this arm different from qwen25_math_1p5b_gxpo_k10.sh:
#
...
[dry-run] resolved launch configuration
  ...
  model              : $MODEL_DIR
  data_root          : $DATA_ROOT
  method             : gxpo + adamw (K=$K, alpha=$REPOSITION_ALPHA)
  ...
  optimizer_state    : $GXPO_OPTIMIZER_STATE_MODE   (probe state s1/s2 discarded; one retained step)
  retention_space    : $GXPO_RETENTION_SPACE      (auto = AdamW direction r = d1/d0)
  run-name tag       : $( [[ "$GXPO_RETENTION_SPACE" == auto ]] && echo "_adamwdir (new arm)" || echo "none (legacy g1/g0 arm)" )

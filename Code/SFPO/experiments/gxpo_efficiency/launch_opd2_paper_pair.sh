...
#
# The OPD^2 A/B pair at the PAPER'S EXACT RECIPE, in two tmux sessions:
#   opd2_only  GPU 0     -- OPD^2 advantage, plain AdamW  (qwen25_1p5b_opd2_nogxpo_control.sh)
#   opd2_gxpo  GPUs 1,2  -- OPD^2 advantage + GXPO        (qwen25_1p5b_opd2_gxpo_adamw_dir_k10.sh)
#
...
# Models are the paper's own: Qwen3-1.7B student in non-thinking mode,
# Qwen3-4B-Instruct-2507 teacher, Qwen3-4B-Base teacher_base. Only the dataset
# differs (this repo's DAPO-math + lighteval-math, not NVIDIA open-reasoning).
...
#   max_completion_length 8192 / max_prompt_length 1024            -> MAX_RESPONSE_LENGTH=16384
#     (DEVIATION: 2x the paper's budget, train AND validation, by user choice)
...
PAPER=(
  ...
  # 2, not 4, at 16384: the student's dense [b, R, V] logits are ~5GB per
  # 16k-token row, so 2 keeps the same peak 4 had at 8192.
  OPD2_MICRO_BATCH_SIZE=2
  ...
  # Teacher + teacher_base resident on their OWN GPU (a Ray actor holding one of
  # GPU_IDS), loaded once disk->GPU; FSDP training runs on the other two.
  OPD2_DEDICATED_GPU=True
...
# Layout: 3 GPUs per run. One (Ray assigns it; in practice the first of GPU_IDS)
# holds the OPD^2 scorer -- teacher + teacher_base resident, loaded once -- and
# the other two run FSDP training. Both arms need all three, so --both runs them
# BACK TO BACK in one tmux session instead of side by side.
GPUS="${OPD2_GPU_IDS:-0,1,2}"
arm_cmd () {  # arm_cmd <log name> <script> <extra env...>
  local name="$1" script="$2"; shift 2
  echo "env ${PAPER[*]} GPU_IDS=$GPUS GPU_COUNT=2 FSDP_SIZE=2 $* ${OPD2_EXTRA_ENV:-} \
...
GXPO_CMD="$(arm_cmd opd2_gxpo qwen25_1p5b_opd2_gxpo_adamw_dir_k10.sh \
...
case "$WHICH" in
  ...
  --both) launch opd2_seq "$ONLY_CMD; $GXPO_CMD" ;;
  *) echo "usage: $0 [--both|--only|--gxpo]" >&2; exit 2 ;;
esac

echo
echo "attach:  tmux attach -t opd2_only | opd2_gxpo | opd2_seq (--both, arms run back to back)"

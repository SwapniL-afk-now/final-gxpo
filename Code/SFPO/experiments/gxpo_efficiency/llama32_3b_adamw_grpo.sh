#!/usr/bin/env bash
# Self-contained Llama 3.2 3B RL launcher: AdamW + METHOD.
...
MODEL_ALIAS="llama32-3b-adamw-grpo"
...
# Hardware guard: this launcher targets Hopper (H200, sm_90). FlashAttention-3
# has no sm_120 build and the memory budgets below assume 141 GiB per card, so
# refuse to start on other architectures instead of failing deep inside vLLM.
if [[ "${SKIP_GPU_ARCH_CHECK:-0}" != "1" ]] && command -v nvidia-smi >/dev/null 2>&1; then
  DETECTED_CC="$(nvidia-smi --query-gpu=compute_cap --format=csv,noheader 2>/dev/null | head -1 | tr -d '[:space:]')"
  if [[ -n "$DETECTED_CC" && "$DETECTED_CC" != 9.* ]]; then
    echo "This launcher targets H200 (sm_90); detected compute capability $DETECTED_CC." >&2
    echo "Use experiments/gxpo_efficiency/llama32_3b_adamw_${METHOD}_blackwell.sh," >&2
    echo "or set SKIP_GPU_ARCH_CHECK=1 to override." >&2
    exit 2
  fi
fi
...
if [[ "grpo" == "gxpo" ]]; then
  ...
  RUN_NAME="${GXPO_RUN_NAME:-llama32_3b_adamw_gxpo_k${K}_a${REPOSITION_ALPHA}_b256_mb64_g4_fsdp4}"
else
  RUN_NAME="${GXPO_RUN_NAME:-llama32_3b_adamw_grpo_b256_mb64_g4_fsdp4}"
fi
...
export WANDB_TAGS="${WANDB_TAGS:-model:$MODEL_ALIAS,method:$METHOD,optimizer:adamw,batch:256,minibatch:64,gpus:4,fsdp:4,experiment:final-efficiency}"

cat <<EOT
[llama AdamW launcher]
model=$MODEL_ID
method=$METHOD

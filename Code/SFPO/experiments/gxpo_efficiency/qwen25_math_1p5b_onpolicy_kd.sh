#!/usr/bin/env bash
#
# qwen25_math_1p5b_onpolicy_kd.sh
#
...
#
# Usage:
#   bash qwen25_math_1p5b_onpolicy_kd.sh            # launch
#   bash qwen25_math_1p5b_onpolicy_kd.sh --dry-run  # preflight only
set -euo pipefail
...
# OOM fix (mirrors the GXPO launcher): expandable_segments crashes vLLM init,
# so cap split sizes instead (see GXPO launcher comment).
...
# KD_SOURCE=onpolicy (default): student rollouts via vLLM, per-step teacher
# scoring. KD_SOURCE=offpolicy_oss: fixed gpt-oss traces + cached teacher
# top-K, EOS-enforced at prep; no vLLM generation, no per-step scoring.
KD_SOURCE="${KD_SOURCE:-onpolicy}"
if [[ "$KD_SOURCE" == "offpolicy_oss" ]]; then
  RUN_NAME="${RUN_NAME:-${MODEL_ALIAS}_offpolicy_kd_oss_b256_topk16}"
else
  RUN_NAME="${RUN_NAME:-${MODEL_ALIAS}_onpolicy_kd_b256_topk16}"
fi
...
# Traditional periodic-eval / checkpoint strategy (mirrors common.sh):
# greedy-decode validation every TRAINER_TEST_FREQ steps on the standard
# 6-benchmark val set via the same vLLM rollout engine (it sleeps again once
# generation finishes and training resumes), the best-scoring checkpoint is
# saved to a single "best_checkpoint" dir that is overwritten whenever a new
# best is found, and the most recent "global_step_N" resumable checkpoint is
# saved every SAVE_FREQ steps with only the latest one kept (keep_last_ckpts=1
# below), replacing the previous global-step checkpoint.
...
[dry-run] resolved on-policy KD-only launch configuration
  ...
  eval/ckpt          : test_freq=$TRAINER_TEST_FREQ (greedy, max_response_length=$MAX_RESPONSE_LENGTH, via vLLM then sleep) \
                        save_freq=$SAVE_FREQ (keep newest global_step_* + best_checkpoint, val_before_train=$VAL_BEFORE_TRAIN)
  val_files          : $VAL_FILES
  run_dir            : $RUN_DIR

#!/usr/bin/env bash
#
# qwen25_math_1p5b_gxpo_muon_faststate_b64_mb16.sh
#
...
#
# Optimizer-state mode: TRANSACTIONAL_FAST_STATE (no refresh) -- the two probe
# steps' Muon/Adam moments and step counter are kept instead of rolled back, so
# the slow correction step is taken from post-probe moments and the step
# counter advances 3x per minibatch instead of 1x. This is the companion arm
# to qwen25_math_1p5b_gxpo_muon_transactional_b64_mb16.sh, which snapshots and
# restores the optimizer state around the two probe steps instead.
#
...
#
# Usage:
#   bash qwen25_math_1p5b_gxpo_muon_faststate_b64_mb16.sh            # launch
#   bash qwen25_math_1p5b_gxpo_muon_faststate_b64_mb16.sh --dry-run  # print
#                                                          # resolved config, no launch
#
# Every setting below can be overridden from the environment, e.g.:
#   GXPO_TAU=1.5 MAX_STEPS=200 bash qwen25_math_1p5b_gxpo_muon_faststate_b64_mb16.sh
set -euo pipefail
...
# No-refresh GXPO: the two probe steps' optimizer moments and step counter are
# kept, and the slow correction step is taken from them.

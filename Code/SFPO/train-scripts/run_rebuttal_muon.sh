#!/usr/bin/env bash
# Muon arm of the rebuttal runs: identical to run_rebuttal_seeded.sh except the actor
# optimizer is Muon (Moonlight impl) instead of AdamW. lr stays 1e-6 -- Moonlight's
# 0.2*sqrt(max(A,B)) adjustment makes Muon's update RMS match AdamW's at the same lr, so
# orthogonalization is the only variable that changes vs the AdamW baseline.
# Usage: METHOD=sfpo GPU=0 TRAIN_SEED=42 ./run_rebuttal_muon.sh
set -euo pipefail
...
EXP="${METHOD}_muon_k${K}_a${ALPHA}_tau${TAU}_mathl35_amc23_seed${TRAIN_SEED}"
RUN_DIR="./runs/${EXP}"
mkdir -p "$RUN_DIR"

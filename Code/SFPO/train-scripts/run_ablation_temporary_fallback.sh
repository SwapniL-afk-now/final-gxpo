#!/usr/bin/env bash
# Group-2 ablation: temporary fallback (revert to GRPO for a window, then re-arm the gate)
# Reviewer question answered: Mvsj -- how sensitive is GXPO to temporary vs. permanent fallback?
# Self-contained -- a reviewer can run this directly:  GPU=1 ./train-scripts/run_ablation_temporary_fallback.sh
...
EXP="gxpo_k${K}_a${ALPHA}_tau${GXPO_TAU}_mathl35_amc23_seed${TRAIN_SEED}_tempfallback"
RUN_DIR="./runs/${EXP}"
mkdir -p "$RUN_DIR"
...
python -u -m verl.trainer.main_ppo \
    ...
    +actor_rollout_ref.actor.gxpo_fallback_mode=temporary \
    +actor_rollout_ref.actor.gxpo_fallback_window=10 \

#!/usr/bin/env bash
# FSDP1 AdamW baseline: stored DeepSeek-R1 math traces -> Qwen2.5-1.5B.
set -euo pipefail
...
export WANDB_RUN_NAME="${WANDB_RUN_NAME:-math_distill_qwen25_1p5b_adamw_r1traces}"
...
if [[ "${1:-}" == "--dry-run" ]]; then
  printf 'model=%s\ntrain=%s\nval=%s\neval=%s (math500,aime24,aime25)\nGPUs=%s FSDP_SIZE=%s batch=256 microbatch=%s steps=%s\nKD=offline_teacher_logits topk=%s weight=%s temperature=%s\neval=greedy n=1 temperature=0 top_p=1\nW&B=%s/%s\n' \
    "$MODEL" "$TRAIN_FILE" "$VAL_FILE" "$EVAL_ROOT" "$CUDA_VISIBLE_DEVICES" "$GPU_COUNT" "$MICRO_BATCH_SIZE_PER_GPU" "$MAX_STEPS" "$KD_TOPK" "$KD_WEIGHT" "$KD_TEMPERATURE" "$WANDB_PROJECT" "$WANDB_RUN_NAME"
  exit 0
fi

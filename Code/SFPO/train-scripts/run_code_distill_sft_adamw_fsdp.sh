#!/usr/bin/env bash
# Two-GPU FSDP1 partition (override GPU_IDS/GPU_COUNT for another placement) AdamW baseline for stored DeepSeek-R1 math-trace distillation.
set -euo pipefail
...
export WANDB_RUN_NAME="${WANDB_RUN_NAME:-math_distill_adamw_gpus0123}"
...
RUN_DIR="${CODE_DISTILL_RUN_ROOT:-$CODE_ROOT/runs/math_distill_sft_adamw}"

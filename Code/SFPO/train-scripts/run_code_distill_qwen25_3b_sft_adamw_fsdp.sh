#!/usr/bin/env bash
# Self-contained Qwen2.5-3B-Instruct offline KD SFT launcher (AdamW/FSDP1).
set -euo pipefail
...
export WANDB_RUN_NAME="${WANDB_RUN_NAME:-code_distill_qwen25_3b_adamw}"
...
RUN_DIR="${CODE_DISTILL_RUN_ROOT:-$CODE_ROOT/runs/code_distill_qwen25_3b_adamw}"
...
if [[ "${1:-}" == "--dry-run" ]]; then
  printf 'model=%s\ntrain=%s/teacher_kd_train.parquet\nval=%s/teacher_kd_val.parquet\nGPUs=%s FSDP_SIZE=%s global_batch=256 microbatch=4 max_steps=%s\nKD=offline_top%s weight=%s temperature=%s\neval_temperature=0.0 eval_top_p=1.0 (greedy)\nW&B=%s/%s\n' \
    "$MODEL" "$DATA_ROOT" "$DATA_ROOT" "$CUDA_VISIBLE_DEVICES" "$FSDP_SIZE" "$MAX_STEPS" "$KD_TOPK" "$KD_WEIGHT" "$KD_TEMPERATURE" "$WANDB_PROJECT" "$WANDB_RUN_NAME"
  exit 0
fi

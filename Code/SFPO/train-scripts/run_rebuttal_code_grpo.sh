#!/usr/bin/env bash
# Non-math RLVR rebuttal run -- GRPO baseline (answers Mvsj "add >=1 non-math RLVR experiment").
...
# AdamW optimizer. Pairs with run_rebuttal_code_gxpo.sh (identical except the GXPO flags).
# Build the data first:  python train-scripts/prep_code_rlvr.py
# Usage: GPU=0 TRAIN_SEED=42 ./train-scripts/run_rebuttal_code_grpo.sh
set -euo pipefail
...
TRAIN=/workspace/jepa-grpo-cache/data/code_rlvr/train.parquet
...
EXP="grpo_code_taco_apps_seed${TRAIN_SEED}"
RUN_DIR="./runs/${EXP}"
mkdir -p "$RUN_DIR"
...
export REWARD_NUM_WORKERS=128      # more parallel test-exec workers (384 CPUs)

#!/usr/bin/env bash
# SFT + GXPO-style update: identical to run_sft_baseline_bespoke_3b.sh except each
# step is the GXPO 3-pass extrapolated update (probe g0, probe g1, reposition,
# slow correction) applied to a plain cross-entropy objective -- no importance
# ratio, no advantages. K=3, alpha=0.5, tau=5.0, warmup=3.
#
# Costs ~3x the baseline's wall time (3 backward passes per step).
# Qwen2.5-3B-Instruct on Bespoke-Stratos-17k, max_length=16384,
# micro_batch_size=1, lr 1e-5, batch 64, 200 steps.
# Usage: GPU=1 ./run_sft_gxpo_bespoke_3b.sh
set -euo pipefail
...
EXP="sft_gxpo_k${K}_a${ALPHA}_tau${GXPO_TAU}_stratos3b_seed${TRAIN_SEED}${OPT_STATE_TAG}"

...
# Prefer the eval_sft_6bench layout (present); fall back to the legacy dapo layout.
if [[ -f "$GXPO_DATA/eval_sft_6bench/math500.parquet" ]]; then
  BENCH_FILES=("$GXPO_DATA/eval_sft_6bench/math500.parquet" "$GXPO_DATA/eval_sft_6bench/aime24.parquet"
    "$GXPO_DATA/eval_sft_6bench/aime25.parquet" "$GXPO_DATA/eval_sft_6bench/amc23.parquet"
    "$GXPO_DATA/eval_sft_6bench/minervamath.parquet" "$GXPO_DATA/eval_sft_6bench/olympiadbench.parquet")
else
...
gpu_free() {
  ...
  if pgrep -f "verl.trainer.(kd_sft_trainer|fsdp_sft_trainer)" >/dev/null 2>&1; then return 1; fi
...
eval_step() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu" "$PYTHON_BIN" tools/kd_sft/evaluate_greedy.py \
    ...
    --data-files "${BENCH_FILES[@]}" --seed 0 --step "$step" \
    --n 1 --temperature 0 --top-p 1.0 --max-tokens 3072 --tp 1 \
    --gpu-memory-utilization 0.85 --max-num-seqs 256 --max-num-batched-tokens 65536 \
    --output "$RUN_DIR/eval_greedy_step${step}.json" 2>&1 | tee "$RUN_DIR/eval_greedy_step${step}.log"
  touch "$marker"
...
if [[ "$WATCH" -eq 1 ]]; then
  ...
  while true; do
    pass_once
    if [[ "$DID_EVAL" -eq 0 ]] && ! pgrep -f "verl.trainer.(kd_sft_trainer|fsdp_sft_trainer)" >/dev/null 2>&1 \

...
def run_score_mode(args, llm, student_tok, pad_id, student_V, rows):
    """Score supplied student responses for an offline KD-SFT cache.

    The teacher teacher-forces the exact student-tokenizer ID sequence and
    returns top-K logprobs per response position.
...
def main():
    ...
    params = SamplingParams(temperature=args.gen_temperature, top_p=args.gen_top_p,
                            ...
                            stop_token_ids=[student_tok.eos_token_id],
                            ignore_eos=False)

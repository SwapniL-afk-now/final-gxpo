...
class TestMuon(unittest.TestCase):
    ...
    def test_actor_has_explicit_probe_and_commit_step_boundaries(self):
        source = (REPO / "verl" / "workers" / "actor" / "dp_actor.py").read_text()
        tree = ast.parse(source)
        optimizer_step_defs = [node for node in ast.walk(tree)
                               if isinstance(node, ast.FunctionDef)
                               and node.name == "_optimizer_step"]
        self.assertEqual(len(optimizer_step_defs), 1)
        gxpo_source = source[source.index("def _gxpo_minibatch_step"):]
        self.assertIn("def probe_optimizer_step", gxpo_source)
        self.assertIn("optimizer_transaction.restore()", gxpo_source)
        self.assertIn("def _optimizer_step(self)", source)

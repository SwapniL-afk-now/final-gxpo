...
class KDSFTDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        # Apply only the student prompt template. The response text is not
        # re-tokenized; response_ids are the exact cache-generation IDs.
        ...
        # This prevents response text detokenize/retokenize drift. Length-capped
        # vLLM generations do not contain EOS, so append exactly one EOS token.
        # The appended token has no cached teacher row; it is trained by CE and
        # deliberately excluded from KL via kd_loss_mask below.
        import numpy as np
        raw_response_ids = np.asarray(self.response_ids[item]).flatten().astype(np.int64)
        if raw_response_ids.size == 0:
            raise ValueError(f'KD cache row {item} has an empty response_ids sequence')
        cached_response_length = int(raw_response_ids.size)
        response_ids = torch.tensor(raw_response_ids, dtype=torch.long)
        eos_id = self.tokenizer.eos_token_id
        if self.include_eos:
            if eos_id is None:
                raise ValueError('include_eos=True requires tokenizer.eos_token_id')
            if int(response_ids[-1]) != int(eos_id):
                response_ids = torch.cat((response_ids, response_ids.new_tensor([int(eos_id)])))
        ...
        # Canvas is aligned to input positions. KDSFTTrainer shifts this
        # mask by one position when selecting teacher rows. Any EOS appended
        # above is therefore intentionally absent from the KL mask.
        kd_loss_mask = torch.zeros_like(attention_mask)
        kd_start = min(prompt_length, kd_loss_mask.size(0))
        kd_end = min(prompt_length + cached_response_length, kd_loss_mask.size(0))
        if kd_end > kd_start:
            kd_loss_mask[kd_start:kd_end] = 1
        ...
        if t_lp.shape[0] != cached_response_length:
            raise ValueError(f'KD cache row {item} has {t_lp.shape[0]} teacher rows, '
                             f'but exact cached response_ids has {cached_response_length}')
        ...
        take = min(max(0, end - start), t_lp.shape[0])
        if take:
            canvas_lp[start:start + take] = torch.from_numpy(t_lp[:take])
            canvas_id[start:start + take] = torch.from_numpy(t_id[:take])

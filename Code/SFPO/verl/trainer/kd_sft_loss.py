...
"""Offline top-K forward-KL distillation loss for KD-SFT.

Implemented separately from the on-policy KD actor path
...
def compute_forward_kl_topk_chunked(
    ...
    ``temperature`` implements the classic Hinton distillation convention:
    both sides are tempered (logits / T) before the divergence, and the
    resulting per-token loss is scaled by T^2 so its gradient magnitude
    matches the untempered (T=1) hard-label loss it is typically combined
    with. The teacher side is only ever available as a top-K log-prob cache
    (never raw logits), so tempering it means dividing the cached top-K
    log-probs by T and renormalizing over just those K entries -- an
    approximation of the true full-vocabulary tempered softmax that is exact
    only insofar as the teacher's off-top-K mass is negligible. This is the
    standard workaround when only a top-K cache is available; it is applied
    before the existing out-of-vocabulary sentinel handling below, which
    still separates sentinel ranks correctly since dividing/renormalizing
    preserves their extreme negativity relative to real entries.
    """
    ...
    for start in range(0, n_tokens, chunk_tokens):
        ...
        if temperature != 1.0:
            # Renormalize over the non-sentinel ranks only: a row with a
            # banned (student-vocab-excluded) rank mixed among real ones must
            # not have that rank's mass folded into the renormalization, and
            # a (structurally near-impossible, but guarded anyway) all-banned
            # row must not be turned into a spurious uniform target by
            # subtracting a finite logsumexp from otherwise-uniform sentinels.
            sentinel = t_topk < -100.0
            t_temp = (t_topk / temperature).masked_fill(sentinel, float('-inf'))
            denom = torch.logsumexp(t_temp, dim=-1, keepdim=True)
            denom = torch.where(torch.isfinite(denom), denom, torch.zeros_like(denom))
            t_topk = torch.where(sentinel, t_topk, t_temp - denom)
